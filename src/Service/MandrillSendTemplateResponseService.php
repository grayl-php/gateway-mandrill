<?php
    ...
    */
   class MandrillSendTemplateResponseService implements ResponseServiceInterface
   {
      ...
      public function getReferenceID ( $response_data ): ?string
      {
         ...
         if ( empty ( $response_data->getAPIResponse() ) || ! is_array( $response_data->getAPIResponse() ) ) {
            // Empty data
            return null;
         }
         ...
         foreach ( $response_data->getAPIResponse() as $recipient ) {
            // Check the ID
       ...
       * @return bool
       */
      private function hasSendFailures ( MandrillSendTemplateResponseData $response_data ): bool
      {
      ...
      private function isRecipientSuccessful ( array $recipient ): bool
      {
         ...
         if ( ! empty( $recipient[ 'status' ] ) && ( $recipient[ 'status' ] == 'sent' || $recipient[ 'status' ] == 'queued' ) ) {
            // Success
            return true;
         }
