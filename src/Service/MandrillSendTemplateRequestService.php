 ...
 */
class MandrillSendTemplateRequestService implements
    RequestServiceInterface
{
    ...
    public function sendRequestDataEntity(
        ...
        // Get the parameters
        $parameters = $this->translateMandrillSendTemplateRequestData(
            $request_data
        );
        ...
        // Send the request
        $response = $api_request->messages->sendTemplate(
            $request_data->getSlug(),
            $content,
            $parameters
        );
    ...
    public function newResponseDataEntity(
        ...
        return new MandrillSendTemplateResponseData(
            $api_response,
            $gateway_name,
            $action
        );
     ...
     * @return array
     */
    private function translateMandrillSendTemplateRequestData(
        MandrillSendTemplateRequestData $request_data
    ): array {
        ...
        $parameters['from_name ']        = $request_data->getSender()
            ->getDisplayName();
        $parameters['to']                = $this->translateRecipients(
            $request_data
        );
        $parameters['global_merge_vars'] = $this->translateMergeTags(
            $request_data->getMergeTags()
        );
        $parameters['tags']              = $request_data->getTags();
        $parameters['merge_vars']        = $this->translateRecipientMergeTags(
            $request_data
        );
     ...
     * @return array
     */
    private function translateRecipients(
        MandrillSendTemplateRequestData $request_data
    ): array {
        ...
        foreach (
            $request_data->getRecipients() as $recipient
        ) {
     ...
     * @return array
     */
    private function translateRecipientMergeTags(
        MandrillSendTemplateRequestData $request_data
    ): array {
        ...
        foreach (
            ...
            // Format this recipient
            $variables[] = [
                'rcpt' => $recipient->getEmailAddress(),
